...
class UnetGaussian(nn.Module):
    def __init__(self, backend, min_val=0.1, max_val=10, gaussian=True, mini_vit=False):
        super(UnetGaussian, self).__init__()
        self.min_val = min_val
        self.max_val = max_val
        initial_mean = 1
        ...
        if mini_vit:
            self.adaptive_bins_layer = mViT(128, n_query_channels=128, patch_size=16,
                                            embedding_dim=128, regression_head=False)
        else:
        ...
        self.conv_out = nn.Conv2d(128, out_dims, kernel_size=1, stride=1, padding=0)

    def forward(self, x, **kwargs):
        out = self.decoder(self.encoder(x), **kwargs)

        if self.adaptive_bins_layer is not None:
            _, out = self.adaptive_bins_layer(out)
    ...
    def get_10x_lr_params(self):  # lr learning rate
        ...
        if self.adaptive_bins_layer:
            modules.append(self.adaptive_bins_layer)
        for m in modules:
            yield from m.parameters()
