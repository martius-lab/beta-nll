...
from src.models.utils import student_t_nll, student_t_from_inv_gamma_nll
from .base_trainer import BaseTrainer
from .hooks import HookPhase


class StudentTTrainer(BaseTrainer):
    def __init__(
        self,
        ...
        self.loss_fn = student_t_from_inv_gamma_nll
    ...
    def train(self, model, dataset, val_dataset, logger):
        ...
        for epoch in range(self.n_epochs):
            ...
            for idx, batch in enumerate(dataloader):
                ...
                loss = self.loss_fn(pred[:3], target)
                if self.model_regularizer is not None:
    ...
    def eval_model(self, model, xs):
        ...
        mean, alpha, beta = model(torch.as_tensor(xs, dtype=torch.float))
